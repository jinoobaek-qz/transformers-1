...
from .modeling_utils import create_position_ids_from_input_ids, SequenceSummary
...
class RobertaForSequenceClassification2(BertPreTrainedModel):
    config_class = RobertaConfig
    base_model_prefix = "roberta"
    ...
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        logger.warn('roberta 2 is being used')
        logger.warn('roberta 2 is being used')
        logger.warn('roberta 2 is being used')
    ...
    Examples::
        ...
        #
        # return outputs  # (loss), logits, (hidden_states), (attentions)
        logits = self.sequence_summary(sequence_output)

        outputs = (logits,) + outputs[2:]  # Keep new_mems and attention/hidden states if they are here
